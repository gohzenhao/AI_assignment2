...
public class ProblemSpecAngle {
	...
	private ASVConfigAngle initialState;
	/** The goal configuration */
	private ASVConfigAngle goalState;
	/** The obstacles */
	private List<Obstacle> obstacles;

	/** The path taken in the solution */
	private List<ASVConfigAngle> path;
	...
	public void loadProblem(String filename) throws IOException {
		...
		try {
			...
			line = input.readLine();
			lineNo++;
			initialState = new ASVConfigAngle(asvCount, line);
//			for(int i=0;i<initialState.getASVPositions().size();i++){
//				System.out.println(initialState.getASVPositions().get(i));
//			}

			line = input.readLine();
			lineNo++;
			goalState = new ASVConfigAngle(asvCount, line);
	...
	public void loadSolution(String filename) throws IOException {
		...
		try {
			...
			path = new ArrayList<ASVConfigAngle>();
			for (int i = 0; i < pathLength; i++) {
				line = input.readLine();
				lineNo++;
				path.add(new ASVConfigAngle(asvCount, line));
			}
			solutionLoaded = true;
		} catch (InputMismatchException e) {
			throw new IOException(String.format(
	...
	public double calculateTotalCost() {
		double cost = 0;
		ASVConfigAngle c0 = path.get(0);
		for (int i = 1; i < path.size(); i++) {
			ASVConfigAngle c1 = path.get(i);
	 ...
	 */
	public ASVConfigAngle getInitialState() {
		return initialState;
	}
	 ...
	 */
	public ASVConfigAngle getGoalState() {
		return goalState;
	}
	 ...
	 *            the new path.
	 */
	public void setPath(List<ASVConfigAngle> path) {
		if (!problemLoaded) {
			return;
		}
		this.path = new ArrayList<ASVConfigAngle>(path);
		solutionCost = calculateTotalCost();
		solutionLoaded = true;
	}
	 ...
	 */
	public List<ASVConfigAngle> getPath() {
		return new ArrayList<ASVConfigAngle>(path);
	}
