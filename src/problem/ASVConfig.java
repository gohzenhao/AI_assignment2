...
public class ASVConfig {
	...
	public void convertToAngle(){
		Point2D firstASV = this.getASVPositions().get(0);
		for(int i=1;i<this.getASVPositions().size();i++){
			float angle = (float) Math.toDegrees(Math.atan2(this.getASVPositions().get(i).getY()-firstASV.getY(), this.getASVPositions().get(i).getX()-firstASV.getX()));
